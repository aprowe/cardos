...
int main(void) {
  ...
  printf("-- alarmfmt --\n");
  RUN(test_alarm_parses_what_it_writes);
  RUN(test_alarm_refuses_nonsense);
  RUN(test_alarm_rings_on_its_days_only);
  RUN(test_alarm_minutes_until_the_next_ring);
  RUN(test_alarm_says_its_days_in_words);
  RUN(test_alarm_reads_a_time_as_people_say_it);
  printf("-- appidx --\n");
  RUN(test_appidx_finds_what_it_wrote);
  RUN(test_appidx_refuses_another_key);
  RUN(test_appidx_misses_a_changed_file);
  RUN(test_appidx_matches_whole_paths);
  RUN(test_appidx_name_cannot_break_the_line);
  RUN(test_appidx_record_that_does_not_fit_writes_nothing);
  RUN(test_appidx_commands_too_long_is_a_miss);
  RUN(test_appidx_torn_file);
  printf("-- appimage --\n");
  ...
  printf("-- calc --\n");
  RUN(test_calc_division_matches_the_hardware);
  RUN(test_calc_functions_agree_with_libm);
  RUN(test_calc_whole_turns_are_exactly_zero);
  RUN(test_calc_numbers_read_like_a_calculator);
  RUN(test_calc_precedence_and_implicit_multiplication);
  RUN(test_calc_variables_and_ans);
  RUN(test_calc_errors_say_what_is_wrong);
  RUN(test_calc_equations_become_graphs);
  RUN(test_calc_typing_and_the_operator_shortcut);
  RUN(test_calc_a_line_is_drawn_where_it_should_be);
  RUN(test_calc_tan_is_not_joined_across_its_asymptote);
  RUN(test_calc_paints_inside_its_rectangle);
  RUN(test_calc_prints_the_roll);
  RUN(test_calc_prints_a_graph_as_a_picture);
  RUN(test_calc_the_window_survives_a_print);
  RUN(test_calc_eval_command);
  printf("-- calendar --\n");
  ...
  RUN(test_calendar_e_opens_the_form_on_the_selected_event);
  RUN(test_calendar_an_edit_is_sent_as_a_patch_and_keeps_the_length);
  RUN(test_calendar_a_new_event_is_still_a_post);
  RUN(test_calendar_an_all_day_event_stays_all_day);
  RUN(test_calendar_a_quote_in_a_title_is_escaped);
  RUN(test_calendar_a_queued_edit_survives_a_fetch);
  RUN(test_calendar_the_form_follows_its_event_through_a_sync);
  RUN(test_calendar_a_patch_to_a_deleted_event_is_dropped);
  RUN(test_calendar_a_failed_patch_keeps_the_edit);
  RUN(test_calendar_an_all_day_event_is_on_its_own_date_anywhere);
  RUN(test_calendar_edit_is_one_of_the_apps_declared_actions);
  printf("-- cfont --\n");
  RUN(test_cfont_parses_the_header);
  RUN(test_cfont_reads_4bpp_coverage);
  RUN(test_cfont_reads_1bpp_as_all_or_nothing);
  RUN(test_cfont_missing_glyph_leaves_a_gap);
  RUN(test_cfont_width_sums_the_advances);
  RUN(test_cfont_refuses_what_would_read_past_the_end);
  RUN(test_cfont_refuses_the_wrong_file);
  RUN(test_cfont_blend_mixes_in_the_panels_byte_order);
  printf("-- chatlog --\n");
  ...
  printf("-- clock --\n");
  RUN(test_clock_add_reads_a_spoken_time);
  RUN(test_clock_off_and_list);
  RUN(test_clock_a_change_does_not_undo_the_kernels);
  printf("-- cmdline --\n");
  ...
  printf("-- counter --\n");
  RUN(test_counter_reads_the_old_file_and_works_out_the_deltas);
  RUN(test_counter_state_round_trips_with_deltas);
  RUN(test_counter_recovers_a_save_stranded_by_a_power_cut);
  RUN(test_counter_ignores_a_half_written_temp);
  RUN(test_counter_does_not_go_below_zero);
  RUN(test_counter_held_space_saves_once_when_it_stops);
  RUN(test_counter_reset_asks_first);
  RUN(test_counter_commands);
  RUN(test_counter_keeps_the_newest_laps);
  printf("-- dav --\n");
  ...
  printf("-- edit preview --\n");
  RUN(test_preview_a_long_line_wraps_and_loses_nothing);
  RUN(test_preview_breaks_at_a_space_when_one_fits);
  RUN(test_preview_the_rows_are_as_full_as_they_can_be);
  RUN(test_preview_a_word_wider_than_the_page_is_broken_inside);
  RUN(test_preview_markers_come_out_and_the_text_stays);
  RUN(test_preview_an_underscore_inside_a_word_is_kept);
  RUN(test_preview_a_fenced_block_is_verbatim_and_wraps_in_6x8);
  RUN(test_preview_a_list_item_wraps_under_its_text);
  RUN(test_preview_without_fonts_still_wraps);
  RUN(test_preview_nothing_is_drawn_past_the_bottom);
  RUN(test_preview_the_last_screen_says_there_is_no_more);
  RUN(test_preview_an_empty_heading_draws_nothing_odd);
  printf("-- forklang --\n");
  RUN(test_forklang_the_default_program_fetches_and_ships);
  RUN(test_forklang_arithmetic_and_precedence);
  RUN(test_forklang_comparison_and_logic);
  RUN(test_forklang_and_or_short_circuit);
  RUN(test_forklang_functions_and_currying);
  RUN(test_forklang_recursion);
  RUN(test_forklang_list_builtins);
  RUN(test_forklang_world_values);
  RUN(test_forklang_continuation_lines_and_comments);
  RUN(test_forklang_a_definition_can_use_a_later_one);
  RUN(test_forklang_parse_errors_name_the_problem_and_line);
  RUN(test_forklang_run_errors_are_errors_not_crashes);
  RUN(test_forklang_runaway_programs_stop);
  RUN(test_forklang_every_run_starts_fresh);
  RUN(test_forklang_the_error_line_is_where_it_went_wrong);
  RUN(test_forklang_completion_lists_builtins_and_definitions);
  printf("-- forklift --\n");
  RUN(test_forklift_the_default_program_earns_money);
  RUN(test_forklift_robots_path_round_racks);
  RUN(test_forklift_the_warehouses_have_what_they_say);
  RUN(test_forklift_shipping_the_wrong_thing_costs);
  RUN(test_forklift_a_finished_order_pays_a_bonus_and_brings_the_next);
  RUN(test_forklift_a_runtime_error_pauses_and_says_where);
  RUN(test_forklift_a_broken_edit_keeps_the_old_program);
  RUN(test_forklift_every_robot_runs_the_program_with_its_own_me);
  RUN(test_forklift_the_shop_takes_credits_and_gives_upgrades);
  RUN(test_forklift_brackets_close_themselves_and_go_as_a_pair);
  RUN(test_forklift_tab_completes_names);
  RUN(test_forklift_enter_indents_after_something_that_wants_more);
  RUN(test_forklift_the_editor_checks_as_you_type);
  RUN(test_forklift_backspace_at_the_start_joins_lines);
  RUN(test_forklift_progress_and_program_survive_a_restart);
  RUN(test_forklift_a_fresh_card_starts_with_a_program_that_works);
  RUN(test_forklift_dump_screens);
  RUN(test_forklift_the_map_never_draws_a_pixel_twice_in_a_step);
  RUN(test_forklift_a_bigger_warehouse_with_three_robots_does_not_flicker);
  RUN(test_forklift_typing_redraws_the_editor_without_clearing_it);
  printf("-- g0gesture --\n");
  ...
  printf("-- habits --\n");
  RUN(test_habits_loads_the_old_files);
  RUN(test_habits_a_save_keeps_the_history_outside_the_window);
  RUN(test_habits_streak_is_not_broken_until_the_day_is_over);
  RUN(test_habits_rate_is_of_the_last_30_days);
  RUN(test_habits_refuses_to_mark_the_future_or_without_a_clock);
  RUN(test_habits_the_window_follows_midnight);
  RUN(test_habits_names_that_cannot_be_files_are_refused);
  RUN(test_habits_rename_moves_the_log_and_delete_removes_it);
  RUN(test_habits_move_reorders_and_saves);
  RUN(test_habits_commands);
  printf("-- hotkeys --\n");
  ...
  printf("-- kvtext --\n");
  RUN(test_kv_reads_pairs_and_forgives_a_hand_edit);
  RUN(test_kv_truncates_rather_than_overruns);
  RUN(test_kv_put_writes_what_next_reads);
  RUN(test_kv_escapes_newlines_in_values);
  printf("-- logring --\n");
  ...
  printf("-- noodle --\n");
  RUN(test_noodle_with_no_air_he_is_a_heap);
  RUN(test_noodle_air_stands_him_up);
  RUN(test_noodle_he_flails_while_the_air_runs);
  RUN(test_noodle_he_sags_when_the_air_stops);
  RUN(test_noodle_stays_on_screen_and_off_the_tarmac);
  RUN(test_noodle_the_render_draws_him_over_the_scene);
  RUN(test_noodle_a_heap_stops_asking_to_be_drawn);
  RUN(test_noodle_the_mic_floor_follows_the_room);
  printf("-- path --\n");
  ...
  RUN(test_printdoc_font_draws_a_line_in_that_font);
  RUN(test_printdoc_font_wraps_by_pixel_width);
  RUN(test_printdoc_font_heading_uses_the_heading_font_and_is_ruled);
  RUN(test_printdoc_font_checkbox_sits_beside_its_text);
  RUN(test_printdoc_no_fonts_is_the_6x8_rendering);
  RUN(test_printdoc_bits_line_lights_the_runs);
  RUN(test_printdoc_bits_line_repeats_and_ends_black);
  RUN(test_printdoc_not_quite_bits_is_text);
  RUN(test_printdoc_bits_line_carries_raw_pixels_after_an_equals);
  printf("-- rect --\n");
  RUN(test_emptiness);
  ...
  RUN(test_rpc_ask_keeps_the_question);
