...
void env_init(void) {
  s_n = 0;
  /* Only the variables CardOS knows about are read back. NVS has no way to
   * enumerate keys in a namespace without also walking every other namespace,
   * and a fixed list is honest about what actually means anything here. */
  load_one("PATH", DEFAULT_PATH);
  load_one("HOME", "/");
  load_one("EDITOR", "edit");
}
