...
static void panel(const char *title, uint16_t accent) {
  /* The whole screen, because the panel is over everything and a shell may
   * have left the clip at one window. Put back at the end: leaving the clip
   * wide is how something later draws outside the box it was given, and the
   * bug that produces is somebody else's window with a smear on it. */
  Rect was = draw_clip();
